...
bool scene_intersect(const raytracing::entities::Ray& ray, const std::vector<std::unique_ptr<const raytracing::entities::Figure>> &figures,
...
namespace entities{
    ...
    Vec3f casting_ray::cast_ray(const Ray &ray,
        ...
        Ray reflect_ray(reflect_dir*N < 0 ? point - N*1e-3 : point + N*1e-3,
                                              reflect_dir, &figures, &lights, depth+1);// offset the original point to avoid occlusion by the object itself (in first param)
...
void render(const char* out_file_path, const std::vector<std::unique_ptr<const entities::Figure>> &figures, const std::vector<entities::Light> &lights) {
